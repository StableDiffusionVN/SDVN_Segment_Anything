...
def groundingdino_predict(
    ...
    def get_grounding_output(model, image, caption, box_threshold):
        ...
        keep = [i for i, phrase in enumerate(phrases) if phrase.strip() != ""]
        if len(keep) == 0:
            return boxes_filt[:0].cpu()
        return boxes_filt[keep].cpu()
...
class GroundingDinoSAMSegment:
    ...
    def main(self, grounding_dino_model, sam_model, image, prompt, threshold, text_threshold):
        res_images = []
        res_masks = []
        for item in image:
